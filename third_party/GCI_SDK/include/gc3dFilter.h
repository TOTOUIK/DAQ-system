...
* @param [in]    startper:有效点的起始百分比，离拟合平面距离超过此百分比将去掉
* @param [in]    endPer:有效点的终止百分比，离拟合平面距离超过此百分比将去掉
* @return
*/
...
/**
* @brief erodeMetaData  根据mask将非感兴趣区域置为0
...
/**
* @brief getMaskImgFromMultiRectLY  专用函数
* @return
*/
DLLEXPORT  void getMaskImgFromMultiRectLY(gc3d::GC3DMetaData& data, const std::vector<GRect>& rects,unsigned char* mask,const int w,const int h);
...
* @param [in]  needToUpdate:  是否需要更新平面，若为真，则每次重新计算零平面，若为假，则以上次计算出的零平面对数据进行旋转，不在重复计算零平面。默认为真
* @return
*/
DLLEXPORT    void zeroPlaneSet(GC3DMetaData& data,std::vector<GRegion>& regions,bool needToUpdate=true);
