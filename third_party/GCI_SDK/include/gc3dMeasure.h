...
* @param [in]   pts:输入的3D点数组，函数将用输入的点进行平面拟合
...
* @param [in]   points:输入的3D点数组，函数将用输入的点进行平面拟合
...
* @param [in]    startPercent：     计算单个点时并不是把_size*_size里的所有点用来拟合一个点，而是根据高度取其中间的一部分点，startPercent即根据高度的中间的起始百分比值
* @param [in]    endPercent：       计算单个点时并不是把_size*_size里的所有点用来拟合一个点，而是根据高度取其中间的一部分点，endPercent即根据高度的中间的终止百分比值
* @return  成功返回 GC3D_SUCCESS 失败返回 GC3D_ALGORITHM_FITPLANE_FAIL
*/
...
* @param [in]    startPercent：         计算result时并不是计算测量面所有的点到基准面距离的平均值，而是根据高度取其中间的一部分点，startPercent即根据高度的中间的起始百分比值
* @param [in]    endPercent：           计算result时并不是计算测量面所有的点到基准面距离的平均值，而是根据高度取其中间的一部分点，endPercent即根据高度的中间的起始百分比值
* @return 成功返回 GC3D_SUCCESS 失败返回 GC3D_ALGORITHM_FITPLANE_FAIL
*/
...
/**
* @brief getTolarence  计算两个区域之间的高度差
* @param [in]   data:                   输入的3D模型
* @param [in]   basePlane:              输入的基准面的区域，区域可以是圆形、矩形或者多边形
* @param [in]   measurePlane:           输入的测量面的的区域，区域可以是圆形、矩形或者多边形
* @param [inout]    result：            输出最终的断差值，根据后续参数设定的不同，结果会有细微差异，计算的过程是直接计算两个区域内Z平均值的差值
...
/**
* @brief getLinePointsDTT  获取两个点连线之间的3D点集合
* @param [in]   data:                   输入的3D模型
* @param [in]   p1:                     输入的第一个点
* @param [in]   p2:                     输入的第二个点
* @param [inout]    pts                 输出的3D点的集合
* @return
*/
...
* @return
*/
DLLEXPORT    void getColorMap(GC3DMetaData& data,unsigned char* img,float& zMin,float& zMax);
...
* @return 两直线的距离
*/
DLLEXPORT  float computeTwoLineDistance(GC3DMetaData& data, const GRegion& reg1, const GRegion& reg2,
                                                              GPoint& p11,GPoint& p12,GPoint& p21,GPoint& p22,GPoint& crossPoint1,GPoint& crossPoint2);
...
* @return 当前区域体积,单位mm³
*/
DLLEXPORT  float getVolume(GC3DMetaData& data,std::vector<gc3d::GRegion>& region,const float minHeight,std::vector<gc3d::GPoint>& validPoints);

/**
* @brief getVolumeUseMask  该函数暂无效
* @param [in]   data:                       输入的3D模型
* @param [in]   mask:                       输入的用于体积的掩膜，
* @param [in]   minHeight:                  输入的用于计算体积的起始高度，当高度小于这个值是不被计算
* @param [inout]   validPoints:             输出的用于计算体积的2D点集合
* @return 当前区域体积
*/
DLLEXPORT  float getVolumeUseMask(GC3DMetaData& data,unsigned char*& mask,unsigned char*& maskResult,const float minHeight);
...
* @return 当前区域体积,单位mm³
*/
DLLEXPORT  float getVolumeUsePoints(GC3DMetaData& data,std::vector<gc3d::GPoint>& pts);
...
* @return
*/
DLLEXPORT  bool convexDetect(gc3d::GC3DMetaData& data, gc3d::GRotationRect& rect,std::vector<GPoint>& linePoints1,
                                                         std::vector<GPoint>& linePoints2,float& glueWidth,float& glueHeight,float& glueVolume,const float spliteHeight=0.05f);
